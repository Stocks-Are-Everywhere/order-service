...
### 🔖 서비스 개요
  ...
  - **주요 역할**: 
    - 사용자 주문 생성 및 처리
    - 주문 데이터 저장 및 관리
    - 주문 검증 이벤트 발행 및 유저 서비스와의 연동
    - 회사 정보(CompanyCode) 조회 및 관리
    - 거래 내역(TradeHistory) 저장 및 조회

</br>

### 🌏 주요 로직
<img width="1157" alt="주문 처리 흐름도" src="" />
...
#### 1️⃣ 주문 생성 및 처리
- 사용자로부터 주문 요청을 받아 처리합니다.
- 주문 유효성 검사를 수행하고 적절한 상태로 저장합니다.

#### 2️⃣ 주문 데이터 관리
- 생성된 주문 데이터를 데이터베이스에 저장하고 관리합니다.
- 주문 상태 변경 및 업데이트를 처리합니다.

#### 3️⃣ 주문 검증 및 이벤트 발행
- 주문 검증 로직을 수행하고 결과를 유저 서비스로 전달합니다.
- 주문 상태 변경 시 관련 이벤트를 발행하여 다른 서비스에 알립니다.

#### 4️⃣ 회사 정보 관리
- CompanyCode를 기반으로 회사 정보를 조회하고 관리합니다.
- 주문 처리 시 필요한 회사 관련 데이터를 제공합니다.

#### 5️⃣ 거래 내역 관리
- 완료된 주문에 대한 거래 내역(TradeHistory)을 생성하고 저장합니다.
- 사용자별, 기간별 거래 내역 조회 기능을 제공합니다.
